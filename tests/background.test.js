...
test('worker: menús, borradores concurrentes, captura opcional y errores', async () => {
  ...
  try {
    ...
    await new Promise(resolve => setImmediate(resolve));
    assert.equal(captures, 0, 'el icono abre el inicio y no captura');
    assert.equal(created.at(-1).url, 'chrome-extension://test/newtab.html');
    assert.equal(Object.keys(await session.get(null)).length, 2, 'abrir el inicio no crea un borrador');
    ...
    await mutex.idle();
    assert.equal(Object.keys(await session.get(null)).length, 3, 'limpia el borrador si falla la apertura');
    ...
    assert.equal(all.length, 3, 'abrir el inicio no agrega accesos');
