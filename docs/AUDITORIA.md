...
No se añadieron host_permissions, <all_urls>, scripting, clipboardRead, downloads, content_scripts, recursos web públicos ni comunicación externa. Para retirar tabs habría que perder la detección global; para retirar unlimitedStorage habría que imponer una cuota total menor o cambiar la estrategia de almacenamiento.

La API de pestañas exige autoridad apropiada para capturas y datos sensibles de pestañas; la implementación usa tabs y activeTab para esas funciones, sin hosts generales. [Referencia oficial de tabs](https://developer.chrome.com/docs/extensions/reference/api/tabs).
