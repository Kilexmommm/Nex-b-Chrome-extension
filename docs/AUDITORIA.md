...
No se añadieron host_permissions generales para sitios, <all_urls>, scripting, clipboardRead, downloads, content_scripts ni recursos web públicos. La extensión declara `identity` y acceso únicamente a `https://www.googleapis.com/` para sincronizar imágenes privadas en Drive, además de `optional_host_permissions` para la captura masiva. Para retirar tabs habría que perder la detección global; para retirar unlimitedStorage habría que imponer una cuota total menor o cambiar la estrategia de almacenamiento.

La API de pestañas exige autoridad apropiada para capturas y datos sensibles de pestañas; la implementación usa tabs y activeTab para las capturas individuales, y el permiso de host opcional para la captura masiva iniciada por el usuario. [Referencia oficial de tabs](https://developer.chrome.com/docs/extensions/reference/api/tabs).
